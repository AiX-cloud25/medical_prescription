...
_FISHBONE_SYSTEM = (
    ...
    "treat all of these as the same shorthand. A common linear form "
    "chains all four together left-to-right, e.g. "
    "'6.8 > 3490 / 1880 < 90k' means Hemoglobin=6.8, then a bracket "
    "opening into the two middle values Total Count=3490 (upper) and "
    "Neutrophil/Blast %=1880 (lower), closing into Platelets=90k on "
    "the right — read the WHOLE chain as ONE cluster of 4 values, not "
    "as unrelated numbers. Labels are sometimes written out beside a "
    ...
    "9200). Platelet count is usually the LARGEST true value in the "
    "cluster once any suffix is accounted for — 4-6 digits, often "
    "comma-separated (e.g. 70,000), or abbreviated with a trailing "
    "'L' for lakh (1L = 100,000, so '4L' is LARGER than 70,000) or a "
    "trailing 'k'/'K' for thousand (1k = 1,000, so '90k' means "
    "90,000). Convert the suffix mentally to compare true magnitude — "
    "never judge by digit count alone, since '90k' has fewer printed "
    "digits than '3490' but represents a far bigger number. The "
    "remaining value is 0-100, often with '%' or a trailing dot — "
    "label it 'Blast %' on oncology/haematology workup pages (MDS, "
    "leukemia, blast monitoring, BMA/BMBx/FISH mentioned nearby), "
    "otherwise 'Neutrophil'; if it is instead a larger absolute count "
    "(hundreds to thousands, e.g. an absolute neutrophil count), "
    "still place it in this same field — do not discard it for not "
    "looking like a percentage.\n"
    ...
    "into '70000', keep 'k'/'K'/'L' suffixes as written). If you "
    "cannot confidently place a value's role, leave that field null "
    "rather than guessing. If the page has no such diagram anywhere, "
